...
const fetchData = async () => {
  ...
  try {
    ...
    if (groupId.value) {
      ...
      members.value = (membersData.value as GroupMemberResponse[]) || []
    }

    // 2. Fetch Lists
    const { data: listsData } = await listLists()
    lists.value = (listsData.value as ListResponse[]) || []
    ...
    if (currentListId.value) {
      ...
      items.value = (itemsData.value as ItemResponse[]) || []
    }
  } catch (error) {
      ...
      <div
        ...
        <div class="flex gap-3">
          <div class="relative flex-1">
            <input v-model="newItemName" @keyup.enter="handleAddItem"
              :disabled="isLoading || !currentListId || isAdding" aria-label="New item name"
          ...
          <button @click="handleAddItem" :disabled="isLoading || !currentListId || isAdding" aria-label="Add item"
