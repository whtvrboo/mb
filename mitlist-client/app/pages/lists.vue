          ...
          <div class="flex -space-x-3">
            <div v-for="(member, idx) in members.slice(0, 3)" :key="member.user_id"
              ...
              <span v-if="member.user?.avatar_url">
                <img :src="member.user.avatar_url" :alt="member.user?.name || 'Member avatar'" class="w-full h-full object-cover" />
              </span>
        ...
        <div class="flex gap-3">
          <div class="relative flex-1">
            <input v-model="newItemName" @keyup.enter="handleAddItem" :disabled="isLoading || !currentListId || isAdding"
              class="w-full h-14 bg-white border-[3px] border-background-dark rounded-lg px-4 text-lg font-medium placeholder:text-gray-400 shadow-neobrutalism-sm focus:outline-none focus:ring-0 focus:shadow-neobrutalism focus:-translate-y-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              placeholder="Add new item..." type="text" />
          </div>
          <button @click="handleAddItem" :disabled="isLoading || !currentListId || isAdding"
            aria-label="Add item"
            class="size-14 bg-primary border-[3px] border-background-dark rounded-lg shadow-neobrutalism-sm flex items-center justify-center hover:bg-[#ffe14f] active:shadow-none active:translate-y-[2px] active:translate-x-[2px] transition-all disabled:opacity-50 disabled:active:translate-x-0 disabled:active:translate-y-0 disabled:shadow-neobrutalism-sm disabled:cursor-not-allowed">
