...
const { getLeaderboard, getSummary, listAchievements, getMyAchievements } =
  useGamification();
    ...
    <main class="flex flex-col gap-6 p-5 max-w-lg mx-auto w-full">
      ...
      >
        <div class="flex flex-col z-10">
          <span
            class="text-xs font-black uppercase tracking-widest opacity-60 text-background-dark"
            >My Rank</span
          >
          <div class="flex items-baseline gap-1">
            ...
            <span class="text-sm font-bold opacity-70"
              >/ {{ leaderboard?.total_participants || "-" }}</span
            >
          </div>
          <div class="mt-2 flex gap-4">
            ...
            <div class="flex flex-col">
              ...
              >
              <span class="font-bold text-lg leading-none">{{
                summary?.achievements_earned || 0
              }}</span>
            </div>
          </div>
        </div>
        <span
          class="material-symbols-outlined text-9xl absolute -right-4 -bottom-4 opacity-10 rotate-12"
          >military_tech</span
        >
      </section>
      ...
      >
        <button
          ...
          class="flex-1 py-3 font-bold uppercase transition-colors"
          :class="
            activeTab === 'rankings'
      ...
      <!-- Leaderboard -->
      <div
        v-if="activeTab === 'rankings' && !isLoading"
        class="flex flex-col gap-3"
      >
        <div
          v-if="!leaderboard?.entries || leaderboard.entries.length === 0"
        ...
        <div
          v-for="(entry, idx) in leaderboard?.entries"
          :key="entry.user_id"
          ...
          :class="{
            'border-primary bg-primary/10': entry.user_id === summary?.user_id,
          }"
        >
          <div
            class="size-8 flex items-center justify-center font-black text-lg text-background-dark/50"
          >
            {{ entry.rank }}
          </div>
          ...
          <div class="flex flex-col flex-1">
            ...
            <span class="text-xs font-bold opacity-50"
              >Level {{ Math.floor(entry.value / 100) + 1 }}</span
            >
          </div>
      ...
      <!-- Achievements -->
      <div
        v-if="activeTab === 'achievements' && !isLoading"
      ...
      >
        <div
          ...
          :class="
            isUnlocked(ach.id)
              ? 'shadow-neobrutalism hover:-translate-y-1'
              : 'opacity-50 grayscale bg-gray-100'
          "
        >
          <div
            class="size-10 rounded-full bg-primary/20 flex items-center justify-center mb-1"
          >
            <span class="material-symbols-outlined text-2xl">{{
              ach.badge_icon_url || "stars"
            }}</span>
          </div>
          <span class="font-bold text-[10px] uppercase leading-tight">{{
            ach.name
          }}</span>
        </div>
      </div>
