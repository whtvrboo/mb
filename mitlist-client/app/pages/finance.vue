...
const formatCurrency = (amount: number, currency = "USD") => {
  // Mock currency for now if not in response
    ...
    <main class="flex flex-col p-5 max-w-lg mx-auto w-full gap-6">
      ...
      >
        <div class="p-6 pb-2 flex flex-col items-center text-center gap-4">
          ...
          >
            <!-- Avatar Placeholer -->
            <div
              class="w-full h-full bg-gray-300 flex items-center justify-center text-2xl font-bold"
            >
              $$
            </div>
          </div>
          <div class="flex flex-col gap-1">
            <span class="text-sm font-bold uppercase tracking-wide opacity-60"
              >Total Balance</span
            >
            <h2 class="text-4xl font-bold leading-none">
              ...
              {{ 0 }}
              <!-- Placeholder for computed balance -->
            </h2>
        ...
        >
          ...
          >
            <span
              class="material-symbols-outlined text-3xl group-hover:scale-110 transition-transform"
              >payments</span
            >
      ...
      <section v-if="!isLoading" class="flex flex-col gap-4">
        <div class="flex items-center justify-between px-1">
          ...
          <button
            class="text-xs font-bold underline decoration-2 underline-offset-2 hover:text-primary transition-colors"
          >
            See All
          </button>
        </div>
        ...
        <div
          ...
          class="text-center py-4 opacity-50 font-bold"
        >
          No expenses yet.
        </div>
        ...
        >
          <div class="flex items-center gap-4">
            <div
              class="size-12 rounded-lg border-[2px] border-background-dark flex items-center justify-center text-background-dark"
            ...
            <div class="flex flex-col">
              ...
              <span class="text-xs font-bold text-gray-500 mt-1"
                >{{ formatDate(expense.date) }} • {{ "Split" }}</span
              >
            </div>
          </div>
