...
<template>
    <nav aria-label="Pagination navigation" class="flex items-center justify-center gap-2" v-if="totalPages > 1">
        <!-- Previous Button -->
        <button @click="goToPage(currentPage - 1)" :disabled="currentPage === 1" aria-label="Go to previous page"
        ...
        <!-- Next Button -->
        <button @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages" aria-label="Go to next page"
            class="flex items-center justify-center size-10 rounded-lg border-[2px] border-background-dark bg-white shadow-[2px_2px_0px_0px_#221f10] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none transition-all">
