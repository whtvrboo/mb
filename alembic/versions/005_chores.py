...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['last_assigned_to_id'], ['users.id'], ),
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.CheckConstraint('quality_rating IS NULL OR (quality_rating >= 1 AND quality_rating <= 5)', name='ck_chore_rating')
    )
    ...
    op.create_table(
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.ForeignKeyConstraint(['chore_id'], ['chores.id'], ),
        sa.ForeignKeyConstraint(['depends_on_chore_id'], ['chores.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
