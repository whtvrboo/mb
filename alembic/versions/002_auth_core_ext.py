...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("preferences", postgresql.JSONB(astext_type=sa.Text()), nullable=True)
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "groups", sa.Column("timezone", sa.String(length=50), nullable=False, server_default="UTC")
    )
    ...
    op.add_column(
        "common_item_concepts", sa.Column("default_category_id", sa.Integer(), nullable=True)
    )
    op.add_column(
        "common_item_concepts", sa.Column("barcode", sa.String(length=100), nullable=True)
    )
    op.add_column("common_item_concepts", sa.Column("average_price", sa.Float(), nullable=True))
    op.add_column(
        "common_item_concepts", sa.Column("image_url", sa.String(length=500), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "role IN ('ADMIN', 'MEMBER', 'GUEST', 'CHILD')", name="ck_user_group_role"
        ),
    )
    ...
    op.create_index(
        op.f("ix_service_contacts_group_id"), "service_contacts", ["group_id"], unique=False
    )

    # Add FK from groups.landlord_contact_id to service_contacts (deferred, will be added after service_contacts exists)
    op.create_foreign_key(
        "fk_groups_landlord_contact", "groups", "service_contacts", ["landlord_contact_id"], ["id"]
    )
