...
async def test_list_chore_history_query_count(db: AsyncSession, engine):
    ...
    It currently uses selectinload with join, causing 2 queries.
    Optimized, it should use contains_eager with join, causing 1 query.
    """
    # 1. Setup Data
    user = User(
        email="history_perf_user@example.com",
    ...
    # 3. Run Function
    history = await list_chore_history(db, group.id)

    # 4. Verify Results
    print(f"Queries executed: {qc.count}")
    assert len(history) == 1

    # Before optimization, this is 2. After optimization, it should be 1.
