...
## 2026-02-01 - Lightweight Accessibility Testing
**Learning:** For components with complex dependencies (like API calls or Auth), mounting them for unit tests is heavy.
**Action:** For static accessibility attributes (aria-labels, roles), use simple file content inspection tests (`fs.readFileSync` + `expect(content).toContain(...)`) to ensure critical a11y features are present without full component rendering.
