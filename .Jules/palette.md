...
## 2026-02-10 - Accessible Modal Pattern
**Learning:** Custom modal implementations often miss standard accessibility features like `role="dialog"`, `aria-modal="true"`, and keyboard navigation (Escape key).
**Action:** Always implement `role="dialog"`, `aria-modal="true"`, associate the title with `aria-labelledby` using `useId()`, and use `@vueuse/core`'s `useEventListener` to handle the Escape key.
