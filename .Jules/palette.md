...
## 2026-02-01 - Modal Accessibility Patterns
**Learning:** Modal components require explicit `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` linked to the title for screen reader support. The Escape key interaction should also be manually handled via event listeners.
**Action:** Updated `UiModal` to include these attributes and behavior, using `useId()` for stable ID generation and `useEventListener` for keyboard support.
