...
## 2026-02-01 - Focus Styles on Custom Inputs
**Learning:** Using `appearance-none` on form inputs (like checkboxes and radios) removes the native browser focus ring, making the element invisible to keyboard users.
**Action:** Always explicitly re-add focus styles (e.g., `focus-visible:ring-2`) when using `appearance-none` to ensure keyboard accessibility.
