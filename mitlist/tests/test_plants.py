...
async def test_create_species_and_plant(authed_client: AsyncClient, auth_headers: dict, db_session):
    ...
    response = await authed_client.post(
        f"/plants/{plant_id}/logs", json=log_data, headers=auth_headers
    )
    assert response.status_code == 201
    ...
    sched_data = {
        ...
        "action_type": "WATER",
        "frequency_days": 7,
        "next_due_date": (datetime.now(UTC) + timedelta(days=7)).isoformat(),
    }
    response = await authed_client.post(
        f"/plants/{plant_id}/schedules", json=sched_data, headers=auth_headers
    )
    assert response.status_code == 201
    sched_id = response.json()["id"]

    # 7. Mark Schedule Done
    done_data = {"notes": "Done via schedule"}
    response = await authed_client.patch(
        f"/plants/schedules/{sched_id}/done", json=done_data, headers=auth_headers
    )
    assert response.status_code == 200
