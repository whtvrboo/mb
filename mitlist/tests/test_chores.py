...
async def test_chores_lifecycle(authed_client: AsyncClient, auth_headers: dict):
    ...
    dep_data = {"chore_id": c2_id, "depends_on_chore_id": c1_id, "dependency_type": "BLOCKING"}
    response = await authed_client.post(
        f"/chores/{c2_id}/dependencies", json=dep_data, headers=auth_headers
    )
    assert response.status_code == 201
    ...
    response = await authed_client.post(
        f"/chores/templates/{tpl_id}/instantiate", json=inst_data, headers=auth_headers
    )
    ...
    response = await authed_client.patch(
        f"/chores/assignments/{asg_id}/start", headers=auth_headers
    )
    assert response.status_code == 200

    # 7.5. Complete Assignment
    complete_data = {"actual_duration_minutes": 30, "notes": "Done"}
    response = await authed_client.patch(
        f"/chores/assignments/{asg_id}/complete", json=complete_data, headers=auth_headers
    )
    assert response.status_code == 200

    # 8. Rate Assignment
    rate_data = {"quality_rating": 5}
    response = await authed_client.post(
        f"/chores/assignments/{asg_id}/rate", json=rate_data, headers=auth_headers
    )
    assert response.status_code == 200
