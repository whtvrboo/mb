...
async def test_assets_lifecycle(authed_client: AsyncClient, auth_headers: dict):
    ...
    response = await authed_client.post(
        f"/assets/{asset_id}/tasks", json=task_data, headers=auth_headers
    )
    assert response.status_code == 201
    task_id = response.json()["id"]

    # 4. Create Log (Complete Task)
    log_data = {"actual_duration_minutes": 10, "notes": "Fast change"}
    response = await authed_client.post(
        f"/assets/tasks/{task_id}/logs", json=log_data, headers=auth_headers
    )
    assert response.status_code == 201
