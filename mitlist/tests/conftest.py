...
from mitlist.modules.governance.models import BallotOption, Proposal, VoteRecord  # noqa: F401
...
async def test_group(db_session: AsyncSession, test_user: User) -> Group:
    ...
    membership = UserGroup(
        user_id=test_user.id, group_id=group.id, role="ADMIN", joined_at=datetime.now(UTC)
    )
