...
async def test_pets_lifecycle(authed_client: AsyncClient, auth_headers: dict):
    ...
    response = await authed_client.post(f"/pets/{pet_id}/schedules", json=sched_data, headers=auth_headers)
    assert response.status_code == 201
    ...
    response = await authed_client.patch(f"/pets/schedules/{sched_id}/done", json=done_data, headers=auth_headers)
    assert response.status_code == 200
