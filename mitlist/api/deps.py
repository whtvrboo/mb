...
async def get_current_user(
    ...
    if user is None:
        ...
        linked_sub = current_prefs.get("zitadel_sub")

        if linked_sub and linked_sub != sub:
            # Critical Security: Prevent account takeover via sub mismatch
            raise UnauthorizedError(
                code="TOKEN_SUB_MISMATCH",
                detail="Token subject does not match linked user identity",
            )

        if not linked_sub:
            # First link (Trust On First Use) or legacy migration
