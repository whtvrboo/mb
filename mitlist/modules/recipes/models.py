...
class Recipe(BaseModel, TimestampMixin):
    ...
    calories_per_serving: Mapped[int | None] = mapped_column(nullable=True)
...
class RecipeIngredient(BaseModel, TimestampMixin):
    ...
    item_concept_id: Mapped[int | None] = mapped_column(
        ForeignKey("common_item_concepts.id"), nullable=True
    )
    is_optional: Mapped[bool] = mapped_column(default=False, nullable=False)
    preparation_note: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class MealPlan(BaseModel, TimestampMixin):
    ...
    assigned_cook_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    servings_planned: Mapped[int | None] = mapped_column(nullable=True)
