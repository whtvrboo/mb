...
class List(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        # ⚡ Bolt: Composite index prevents in-memory sorting when listing group lists ordered by ID
...
class InventoryItem(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        # ⚡ Bolt: Composite index prevents in-memory sorting when listing group inventory ordered by ID
        Index("ix_inventory_items_group_id_id", "group_id", "id"),
    )
