...
async def cast_vote(
    ...
    # Ensure deadline_at is timezone-aware if comparing to timezone.utc
    if proposal.deadline_at:
        d_tz = proposal.deadline_at.tzinfo
        deadline_tz = (
            proposal.deadline_at.replace(tzinfo=UTC) if d_tz is None else proposal.deadline_at
        )
        if deadline_tz < now:
            raise ValidationError(code="VOTING_CLOSED", detail="Proposal deadline has passed")
