...
class AuditLog(BaseModel, TimestampMixin):
    ...
    # ⚡ Bolt: Composite index on polymorphic relationship + sort col to prevent memory sorting.
    __table_args__ = (
        Index("ix_audit_logs_entity_type_id_time", "entity_type", "entity_id", "occurred_at"),
    )
...
class TagAssignment(BaseModel, TimestampMixin):
    ...
    # ⚡ Bolt: Composite index prevents single-ID scans when IDs collide across entity types.
    __table_args__ = (Index("ix_tag_assignments_entity_type_id", "entity_type", "entity_id"),)
