...
async def list_assets(db: AsyncSession, group_id: int) -> list[HomeAsset]:
    ...
    result = await db.execute(
        select(HomeAsset)
        .where(HomeAsset.group_id == group_id)
        .order_by(HomeAsset.name)
    )
...
async def update_asset(
    ...
    if asset_type is not None: asset.asset_type = asset_type
    if location_id is not None: asset.location_id = location_id
    if brand is not None: asset.brand = brand
    if model_number is not None: asset.model_number = model_number
    if serial_number is not None: asset.serial_number = serial_number
    if purchase_date is not None: asset.purchase_date = purchase_date
    if purchase_price is not None: asset.purchase_price = purchase_price
    if purchase_store is not None: asset.purchase_store = purchase_store
    if warranty_end_date is not None: asset.warranty_end_date = warranty_end_date
    if warranty_type is not None: asset.warranty_type = warranty_type
    if energy_rating is not None: asset.energy_rating = energy_rating
    if photo_url is not None: asset.photo_url = photo_url
    if manual_document_id is not None: asset.manual_document_id = manual_document_id
    if receipt_document_id is not None: asset.receipt_document_id = receipt_document_id
    if service_contact_id is not None: asset.service_contact_id = service_contact_id
...
async def list_maintenance_tasks(db: AsyncSession, asset_id: int) -> list[MaintenanceTask]:
    ...
    result = await db.execute(
        select(MaintenanceTask)
        .where(MaintenanceTask.asset_id == asset_id)
    )
    return list(result.scalars().all())
...
async def update_maintenance_task(
    ...
    if frequency_days is not None: task.frequency_days = frequency_days
    if priority is not None: task.priority = priority
    if instructions is not None: task.instructions = instructions
    if estimated_duration_minutes is not None: task.estimated_duration_minutes = estimated_duration_minutes
    if estimated_cost is not None: task.estimated_cost = estimated_cost
    if required_item_concept_id is not None: task.required_item_concept_id = required_item_concept_id
    if is_active is not None: task.is_active = is_active
...
async def list_insurances(db: AsyncSession, group_id: int) -> list[AssetInsurance]:
    ...
    result = await db.execute(
        select(AssetInsurance)
        .where(AssetInsurance.group_id == group_id)
    )
    return list(result.scalars().all())
...
async def update_insurance(
    ...
    if policy_number is not None: ins.policy_number = policy_number
    if provider_name is not None: ins.provider_name = provider_name
    if coverage_type is not None: ins.coverage_type = coverage_type
    if premium_amount is not None: ins.premium_amount = premium_amount
    if premium_frequency is not None: ins.premium_frequency = premium_frequency
    if end_date is not None: ins.end_date = end_date
    if deductible_amount is not None: ins.deductible_amount = deductible_amount
    if document_id is not None: ins.document_id = document_id
